...
folderPath = "markdown"  # 設定資料路徑
files = os.listdir(folderPath)  # 取得資料內所有檔案
# 這時候資料夾可能包含其他檔案，我們只需要 .md 檔案
files_name = []  # 新增列表用來存放 .md 的檔案
for f in files:  # 逐一檢查所有檔案，看看是否已 .md 結尾
...
for f in files_name:  # ['class1.md','class2.md]逐一讀取所有 .md 檔案
    ...
    with st.expander(f[:-3]):  # 使用expender，標題為檔案名稱去掉前面的 .md
        st.markdown(content)  # 將檔案內容輸出到網頁
